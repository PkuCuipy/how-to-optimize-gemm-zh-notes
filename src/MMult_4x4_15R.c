...
void AddDot4x4(int k, double* a, int lda, double* b, int ldb, double* c, int ldc) {
    ...
    __m128d c_00_c_10_vreg, c_01_c_11_vreg, c_02_c_12_vreg, c_03_c_13_vreg,  // C
    ...
    for (p = 0; p < k; p++) {
        ...
        a_0p_a_1p_vreg = _mm_load_pd( a );   // pd: packed double
        a_2p_a_3p_vreg = _mm_load_pd( a + 2 );
        a += 4;

        b_p0_vreg = _mm_loaddup_pd( b );   // loaddup: load & duplicate
        b_p1_vreg = _mm_loaddup_pd( b + 1 );   
        b_p2_vreg = _mm_loaddup_pd( b + 2 );   
        b_p3_vreg = _mm_loaddup_pd( b + 3 );   
        b += 4;
    ...
    _mm_store_pd(C_incr + 0, c_00_c_10_vreg);
    _mm_store_pd(C_incr + 2, c_20_c_30_vreg);
    _mm_store_pd(C_incr + 4, c_01_c_11_vreg);
    _mm_store_pd(C_incr + 6, c_21_c_31_vreg);
    _mm_store_pd(C_incr + 8, c_02_c_12_vreg);
    _mm_store_pd(C_incr + 10, c_22_c_32_vreg);
    _mm_store_pd(C_incr + 12, c_03_c_13_vreg);
    _mm_store_pd(C_incr + 14, c_23_c_33_vreg);
