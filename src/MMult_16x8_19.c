...
void AddDot16x8(int k, double* a, double* b, double* c, int ldc);
...
void InnerKernel(int m, int n, int k, double *a, int lda, 
    ...
    for (j = 0; j < n; j += 8) {
        ...
        for (i = 0; i < m; i += 16) {
            ...
            AddDot16x8(k, &packedA[i * k], &packedB[k * j], &C(i, j), ldc);  
        }
    }
}
...
void AddDot16x8(int k, double* a, double* b, double* c, int ldc) {
    //
    // 计算矩阵 C 的如下共 16 x 8 = 64 个元素:
    //
    //   C(0,0)    C(0,1)  ...  C(0,7)
    //   C(1,0)    C(1,1)  ...  C(1,7)
    //   C(2,0)    C(2,1)  ...  C(2,7)
    //      :        :            :
    //   C(15,0)  C(15,1)  ...  C(15,7)
    //
    ...
    // 写回内存
    double C_incr[128] __attribute__((aligned(64)));
    _mm512_store_pd(C_incr + 0, c00__c70_vreg);
    _mm512_store_pd(C_incr + 8, c80__cf0_vreg);
    _mm512_store_pd(C_incr + 16, c01__c71_vreg);
    _mm512_store_pd(C_incr + 24, c81__cf1_vreg);
    _mm512_store_pd(C_incr + 32, c02__c72_vreg);
    _mm512_store_pd(C_incr + 40, c82__cf2_vreg);
    _mm512_store_pd(C_incr + 48, c03__c73_vreg);
    _mm512_store_pd(C_incr + 56, c83__cf3_vreg);
    _mm512_store_pd(C_incr + 64, c04__c74_vreg);
    _mm512_store_pd(C_incr + 72, c84__cf4_vreg);
    _mm512_store_pd(C_incr + 80, c05__c75_vreg);
    _mm512_store_pd(C_incr + 88, c85__cf5_vreg);
    _mm512_store_pd(C_incr + 96, c06__c76_vreg);
    _mm512_store_pd(C_incr + 104, c86__cf6_vreg);
    _mm512_store_pd(C_incr + 112, c07__c77_vreg);
    _mm512_store_pd(C_incr + 120, c87__cf7_vreg);
